# 飞书多维表格 & 云文档 Go SDK 示例

这是一个完整的 Go 语言实现，用于操作飞书多维表格（Lark Base）和云文档（Feishu Docs），支持创建、读取、更新和删除记录等操作。
...
├── main_create.go       # 创建表格并写入数据 ⭐️
├── main_docs.go         # 云文档操作示例 ⭐️ 新增
├── main_export.go       # 数据导出示例（表格→文档）⭐️ 新增
├── config.yaml          # 配置文件
├── go.mod               # Go 模块配置
├── README.md            # 说明文档
...
创建和操作飞书云文档，支持添加标题、文本、列表、代码块等多种内容。
...
程序会自动：
1. 创建云文档
2. 添加各种类型的内容（标题、文本、列表、代码等）
3. 读取文档信息和内容
4. 展示完整的操作流程

详细说明请查看 [DOCS_GUIDE.md](DOCS_GUIDE.md)

---

### 方式二：数据导出（多维表格 → 云文档）⭐️ 新增

将多维表格的数据导出为格式化的云文档报告。
...
feishu:
  app_id: "你的app_id"
  app_secret: "你的app_secret"
  app_token: "你的多维表格app_token"  # 必填
  table_id: "你的table_id"           # 必填
  folder_token: ""                    # 可选
```
...
适用场景：数据报告生成、定期数据汇总、数据存档等。

---

### 方式三：创建新的多维表格（推荐用于测试）⭐️
...
- ✅ **查看、编辑、管理多维表格** (`bitable:app:readonly` 和 `bitable:app`)
  - 在"权限管理" -> "多维表格"中开启
- ✅ **查看、编辑、管理云文档** (`docx:document`) - 如需使用云文档功能 ⭐️
  - 在"权限管理" -> "云文档"中开启
